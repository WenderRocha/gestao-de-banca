...
<template>
    <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div class="flex justify-end gap-3 px-4 pt-4">
            <Button @click="deleteWallet(props.wallet.id)" variant="secondary"
                class="items-center gap-2 max-w-xs flex justify-center">
                <span>Deletar</span>
            </Button>

            <EditWallet @currency-format-update="currencyFormatUpdated" :wallet="props.wallet" />
        </div>
        <div class="flex flex-col items-center pb-10">
            <div class="w-30 h-30 mb-3 rounded-full shadow-lg">
                <img src="/img/app-wallet-passes.png" alt="Bonnie image" />
            </div>
            <h5 class="mb-1 flex gap-2 text-xl font-medium text-gray-900 dark:text-white">
                {{ props.wallet.name }}

                <svg v-show="props.wallet.main" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            ...
            <span class="text-sm text-green-500 font-bold dark:text-green-400">
                <span>
                    <CurrencyFormat :key="refreshCurrencyFormat" :currency="props.wallet.currency"
                        :balance="props.wallet.balance" />
                </span>
            </span>
            <div class="flex mt-4 space-x-3 md:mt-6">
                <Button :href="route('management.index', props.wallet.id)" variant="primary" class="items-center gap-2 max-w-xs">
                    <span>Gerênciar</span>
                </Button>
                <a href="#"
                    class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-700 dark:focus:ring-gray-700">Transação</a>
            </div>
        </div>
    </div>
</template>
